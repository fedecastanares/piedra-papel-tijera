...
export const getGamesRequest = async () => {
    try {
        const response = await axiosApiInstance.get(`${process.env.REACT_APP_PROTOCOL}://${process.env.REACT_APP_BASE_URL}/games`)
...
export const responseGame = async (id, rivalPlay) => {
    ...
    try {
        const response = await axiosApiInstance.post(`${process.env.REACT_APP_PROTOCOL}://${process.env.REACT_APP_BASE_URL}/games/${id}`, data
        )
